...
func NewIPCClient(socket string, done chan string) (*IPCClient, error) {
	c := &IPCClient{
		done: done,
		closed: false,
		socket:  socket,
...
func (c *IPCClient) dispatch(resp *Response) {
	if resp.Event == "" { // No Event
		...
		if strings.EqualFold(resp.Event,"end-file") {
			c.done <- "cmd"
		}
		if strings.EqualFold(resp.Event,"fullscreen") {
			c.done <- "cmd"
		}
	}
}

func (c *IPCClient) run() (*IPCClient, error) {
